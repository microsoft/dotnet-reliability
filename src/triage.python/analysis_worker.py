...
from dumpling_util import CheckTheseNamedPaths
...
site.addsitedir(pathof('lldb_py'))
import lldb
...
def StartDebugger(pathTuple):
    ...
    Logging.Verbose('SOS PLUGIN PATH: ' + pathof('sos_plugin'))
    ...
    if target:
      ...
      if process:
        ...
        Logging.Verbose('plugin load %s' % pathof('sos_plugin'))

        interpreter.HandleCommand(str('plugin load %s' % pathof('sos_plugin')), loadPluginResult)
...
def RunAnalysis(debuggerTuple):
    ...
    Logging.Verbose('Return objects prepared. Asking lldb to import %s' % pathof('analysis.py'))
    interpreter.HandleCommand(str('command script import %s' % pathof('analysis.py')), importScriptCommandResult)
    if importScriptCommandResult.Succeeded():
        Logging.Verbose('analysis.py imported. Running analyze.')
        interpreter.HandleCommand(str('analyze -i %s -o ./analysis.txt' % pathof('triage.ini')), analyzeCommandResult)
...
if __name__ == '__main__':
    ...
    required_files = { 
                'triage.ini':       path.join(_thisFolder, 'triage.ini'),
                'analysis.py':      path.join(_thisFolder, 'analysis.py'),
                'sos_plugin':      str(_config.CONFIG_sos_plugin),
                'lldb_py':         str(_config.CONFIG_LLDB_PYTHON_PATH)
            }

    CheckTheseNamedPaths(required_files)
